...
import com.samsung.ssc.adapters.SellOutProjectionAddAdapter;
...
public class SellOutProjection extends BaseActivity implements LoaderCallbacks<Cursor> {

	private ListView mOrderBooking, mOrderBookingAdd;
	private AlertDialog helpDialog;
	private OrderBookingAdapter adpter;
	private SellOutProjectionAddAdapter adapterAdd;
	...
	private final int LOADER_ID2 = 2;
	private final int LOADER_ID3 = 3;
	private final int LOADER_ID4 = 4;
...
/*	private static class InsertDataHandler extends Handler {
		...
		WeakReference<SellOutProjection> selloutProjection;

		public InsertDataHandler(SellOutProjection selloutProjection) {

			this.selloutProjection = new WeakReference<SellOutProjection>(
					selloutProjection);
		}
		...
		public void handleMessage(Message msg) {
			...
			SellOutProjection selloutProjectionInstance = selloutProjection
					.get();
			...
			if (isSuccess) {
				...
				selloutProjectionInstance.finish();

				if (syncStatus == SyncUtils.SYNC_STATUS_SAVED) {

					Helper.showCustomToast(selloutProjectionInstance,
...
/*	private static class RetriveHandler extends Handler {
		...
		public RetriveHandler(SellOutProjection selloutProjection) {
		...
		public void handleMessage(Message msg) {
			...
			if (selloutProjectionInstance != null
					&& selloutProjectionInstance.sellOutProjectionData.size() > 0) {

				selloutProjectionInstance.addHeaderFooter();

				selloutProjectionInstance.adapterAdd = new SellOutProjectionAddAdapter(
						selloutProjectionInstance,
						selloutProjectionInstance.mActivityID);

				selloutProjectionInstance.mOrderBookingAdd
						.setAdapter(selloutProjectionInstance.adapterAdd);

				selloutProjectionInstance.refreshTotal();
			}
...
*/
	...
	public void init() {
		...
		if (mModuleCode != 0) {
			if (mModuleCode == ModuleCode.MENU_SELL_OUT_PROJECTION) {
				((TextView) this.findViewById(R.id.tv_title_sdActionBar))
						.setText("Sellout Projection");
			}

		}
		
		getSupportLoaderManager().initLoader(LOADER_ID, null, SellOutProjection.this);

		prodCategoryP1 = DatabaseHelper.getConnection(SellOutProjection.this)
				.getProductTypeList();
				...
				if (arg2 > 0) {
					...
					if (loader2 == null) {
						loader2 = getSupportLoaderManager().initLoader(
								LOADER_ID2, bundle, SellOutProjection.this);
					} else {
						getSupportLoaderManager().restartLoader(LOADER_ID2,
								bundle, SellOutProjection.this);
					}
				...
				if (arg2 > 0) {
					...
					if (loader4 == null) {
						loader4 = getSupportLoaderManager().initLoader(
								LOADER_ID4, bundle, SellOutProjection.this);
					} else {
			...
			Thread thread = new Thread(new Runnable() {
				...
				public void run() {
					...
					JSONArray jArray = DatabaseHelper.getConnection(
							getApplicationContext()).getOrderRespose(
							mActivityID, ORDER_TYPE, true);
	...
	public void onClick(View v) {
		...
		case R.id.add:
			...
			if (sellOutProjectionData.size() < 1) {

				Helper.showCustomToast(SellOutProjection.this,
						R.string.qtyempty, Toast.LENGTH_LONG);
			...
			adapterAdd = new SellOutProjectionAddAdapter(this, mActivityID);
	 ...
	 */
	private void onButtonTakeAction(final SellOutProjectionArrayList list,
			int syncStatus) {
...
*/
			...
			public void run() {
				...
				try {
					...
					boolean result = DatabaseHelper.getConnection(
							getApplicationContext()).insertOrderRespnse(
							mActivityID, ORDER_TYPE, jDataArray);
					Message msg = handler.obtainMessage();
					Bundle bundle = new Bundle();
					bundle.putBoolean("is_success", result);
	...
	*/}
	...
	private void backcall() {
		...
		if (sellOutProjectionData != null && sellOutProjectionData.size() > 0) {

			Helper.showAlertDialog(
					SellOutProjection.this,
	...
	/**
	 * Get product list form database
	 * 
	 * @author d.ashish
	 * 
	 */
	...
	public void refreshTotal() {
		...
		if (sellOutProjectionData.size() > 0) {
			...
			double total = 0;
			int quentity = 0;
			for (int i = 0; i < sellOutProjectionData.size(); i++) {
				int qty = Integer
						.parseInt(((SKUProductList) sellOutProjectionData
								.get(i)).getQuantity());
				double tot = Double
						.parseDouble(((SKUProductList) sellOutProjectionData
		...
		if (loaderId.getId() == 1) {
			...
			if (cursor != null && cursor.getCount() > 0) {
				...
				if (sellOutProjectionData.size() > 0) {
					...
					adapterAdd = new SellOutProjectionAddAdapter(
							SellOutProjection.this, mActivityID);
